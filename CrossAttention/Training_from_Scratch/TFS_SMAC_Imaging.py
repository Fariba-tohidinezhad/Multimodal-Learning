# ============================================================
# SMAC_Scratch_ImagingOnly.py
#
# SMAC3 HPO for downstream GIST IMAGING-ONLY model trained FROM SCRATCH
# (ablation analysis; no clinical branch, no cross-attention, no pretraining, no LoRA).
#
# Uses:
...
RUNS_DIR_NAME = "runs_smac_imgonly_scratch"
TEST_CENTER = "EMC"

SMAC_OUTPUT_DIR = "output_smac_imgonly_scratch"
SMAC_RUN_NAME = "gist_imgonly_smac_scratch"
...
def config_slug(cfg: Dict[str, Any], max_len: int = 140) -> str:
    ...
    for k in order:
        ...
        if k == "patch_size_zyx":
            val = fmt_ps(v)
        elif k in ("emb_dim", "depth_b", "heads_b", "imaging_aug_deg"):
            val = str(int(v))
        elif k in ("lr", "wd"):
            val = fmt_lr(v)
        elif k == "mlp_layers":
...
class ImagingOnlyTransformerEncoder(nn.Module):
    """
    Imaging-only transformer backbone for patchified 3D tumor volumes.

    Input:
        xb: [batch, num_patches, patch_dim]
        embed_mask_b: optional [batch, num_patches]

    Output:
        token features [batch, num_patches, emb_dim]
    """
    ...
    def forward(
        self,
        xb: torch.Tensor,
        embed_mask_b: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        x = self.enc_b(xb, return_embeddings=True, mask=embed_mask_b)
...
def build_model(
    ...
    backbone = ImagingOnlyTransformerEncoder(
        emb_dim=cfg["emb_dim"],
...
def build_dataloaders_from_ids(
    ...
    # NOTE:
    # We still use the same dataset class and clinical CSV because:
    # - labels come from the CSV
    # - center/sample_id bookkeeping comes from the same pipeline
    # - the model simply ignores the clinical input tensors
    train_dataset = GISTDataset.dataset(
        ...
        dropout_p=0.0,   # imaging-only ablation -> no clinical augmentation
        train=True,
