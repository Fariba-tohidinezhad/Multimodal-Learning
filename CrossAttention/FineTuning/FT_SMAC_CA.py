...
# SMAC_LoRA_EncB.py
#
# SMAC3 HPO for downstream GIST model using pretrained imaging encoder (enc_b)
# with LoRA applied to enc_b Attention projections "qv".
#
# This version updates:
#   - objective / cost:
#       cost = 1 - mean(fold_val_aucs)
#   - search space:
#       refined to be broader in meaningful directions while staying practical
#       for your compute budget
#
# Keeps the same:
...
RUNS_DIR_NAME = "runs_smac_ca"
TEST_CENTER = "EMC"

SMAC_OUTPUT_DIR = "output_smac_ca"
SMAC_RUN_NAME = "gist_crossattn_smac_lora"
...
def config_slug(cfg: Dict[str, Any], max_len: int = 140) -> str:
    ...
    abbr = {
        ...
        "lora_r": "r", "lora_ratio": "rr", "lora_target": "t",
    }
    order = [
        "lr", "wd",
        "depth_a", "heads_a", "depth_cross_attn", "heads_cross",
    ...
    for k in order:
        ...
        if k in ("lr", "wd"):
            val = fmt_lr(v)
        elif k in ("depth_a", "heads_a", "depth_cross_attn", "heads_cross", "lora_r", "imaging_aug_deg"):
            val = str(int(v))
        elif k == "mlp_layers":
            val = fmt_mlp(v)
        elif k == "clinical_aug":
...
def build_model(
    ...
    backbone = CrossAttentionTransformerEncoder(
        ...
        depth_cross_attn=cfg.get("depth_cross_attn", 1),
...
def create_configspace(base_cfg: Dict[str, Any]) -> ConfigurationSpace:
    ...
    heads_cross_hp = Categorical("heads_cross", [1, 2, 4], default=base_cfg["heads_cross"])
...
def train_one_trial(
    ...
    # Best validation AUC from metrics.csv
    val_auc = 0.0
...
def main() -> None:
    ...
    def smac_objective(config: Configuration, seed: int = 0) -> float:
        ...
        # Aggregate dev-wide predictions for reporting only
