...
# Clinical-only specific run dirs / names
RUNS_DIR_NAME = "runs_smac_clinical_only"
TEST_CENTER = "EMC"

SMAC_OUTPUT_DIR = "smac3_output_clinical_only"
...
def compute_max_num_tokens(largest_tumor: Tuple[int, int, int], patch_size: Tuple[int, int, int]) -> int:
    """
    Kept for compatibility, not used in clinical-only model.
    """
...
class ClinicalTransformerEncoder(nn.Module):
    """
    Clinical-only backbone using TransformerWrapper + Encoder,
    equivalent to the 'enc_a' stream in your CrossAttentionTransformerEncoder,
    but without any cross-attention or imaging stream.

    Input via ModelMultiHead.backbone_args:
        xa            <- batch_dict['data.input.clinical.vector']   [B, N_a]
        embed_ids_a   <- batch_dict['model.embed_ids_a']            dict[str, Tensor]
        embed_mask_a  <- batch_dict['model.embed_mask_a']           [B, N_a]

    Output:
        [B, N_a, D] token embeddings
    """
    def __init__(
        self,
        emb_dim: int,
        ...
        self.enc_a = TransformerWrapper(
            num_tokens=len(embed_num_tokens),
...
def build_model(
    ...
    """
    Clinical-only model:
    - ClinicalTransformerEncoder backbone (sequence of clinical tokens)
    ...
    backbone = ClinicalTransformerEncoder(
        emb_dim=cfg["emb_dim"],
...
def build_dev_test_manifest(
    ...
    NOTE: We still use the same dataset construction (incl. imaging preprocessing)
    to keep splits identical to multimodal runs, but the clinical-only model ignores imaging.
    """
...
def build_dataloaders_from_ids(
    ...
    NOTE: Dataset still runs imaging preprocessing for consistency,
    but the clinical-only model ignores imaging outputs.
    """
...
# =============================
# SMAC: ConfigSpace & mapping (CLINICAL-ONLY)
# =============================

def create_configspace(base_cfg: Dict[str, Any]) -> ConfigurationSpace:
    """
    Clinical-only ConfigSpace:
    - Tune clinical encoder & generic training params
    - Keep imaging-related knobs fixed from base_cfg (not optimized)
    """
...
def cfg_from_config(config: Configuration, base_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    robust to missing imaging params (clinical-only).
    """
...
def train_one_trial(
    ...
    """
    Train a clinical-only model for a single config on a single CV fold and single seed.
    Returns the validation AUC (float) from metrics.csv (best epoch).
...
def load_model_from_checkpoint(
    ...
    """
    Build clinical-only model from cfg and load weights from a Lightning checkpoint.
...
def main():
    ...
    # NOTE: imaging-related knobs kept for dataset construction, but not tuned.
    base_cfg = {
        "patch_size": (8, 64, 64),
        "emb_dim": 64,
        "lr": 1e-4,
    ...
    def smac_objective(config: Configuration, seed: int = 0) -> float:
        ...
        for fold_idx, (train_ids, val_ids) in enumerate(cv_splits):
            ...
            print(f"\n[SMAC] (clinical-only) Config slug={slug} | seed={seed} | CV fold={fold_idx}")
            report = {k: cfg.get(k) for k in REPORT_KEYS if k in cfg}
            print(f"[SMAC] (clinical-only) CONFIG -> {report}")
            ...
            print(f"[SMAC] (clinical-only) slug={slug} | seed={seed} | fold={fold_idx} | val_auc={val_auc:.4f}")
        ...
        print(
            f"[SMAC] (clinical-only) slug={slug} | seed={seed} | "
    ...
    with open(os.path.join(runs_root, f"incumbent_clinical_only_{incumbent_slug}.json"), "w") as f:
        json.dump(incumbent_cfg, f, indent=2)

    print("\n[SMAC] (clinical-only) Optimization finished.")
    print(f"[SMAC] (clinical-only) Incumbent slug: {incumbent_slug}")
    print(f"[SMAC] (clinical-only) Incumbent config: {incumbent_cfg}")
