...
RUNS_DIR_NAME = "runs_smac"
TEST_CENTER = "EMC"

SMAC_OUTPUT_DIR = "smac3_output_fixed"
SMAC_RUN_NAME = "gist_crossattn_smac"
...
def build_dev_test_manifest(
    ...
    full_dataset = GISTDataset.dataset(
        ...
        angle_range=(0.0, 0.0),
        mask_pad_threshold=0.8,  # just something reasonable; no effect on IDs
        dropout_p=0.0,
    )
...
def create_configspace(base_cfg: Dict[str, Any]) -> ConfigurationSpace:
    ...
    # augmentation
    ca_hp = Categorical("clinical_aug", [0.0, 0.1, 0.2], default=base_cfg["clinical_aug"])

    # imaging augmentation rotation degrees
...
def cfg_from_config(config: Configuration, base_cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Merge SMAC config with base_cfg and decode any encoded fields.
    """
    cfg = copy.deepcopy(base_cfg)
    conf = dict(config)

    # patch_size is encoded as "pzxpyxpx"
    ...
    # fixed test center for this SMAC setup
...
def train_one_trial(
    ...
    """
    Train a model for a single config on a single CV fold and single seed.
    Returns the validation AUC (float) based on best epoch.
    The run_dir will contain:
        - best_epoch.ckpt
        - last.ckpt
        - metrics.csv
        - config.json
    ...
    # Save config used for this trial
    ...
    # ------------------------------------------------------------
    # Best checkpoint path: always 'best_epoch.ckpt' in run_dir
    # ------------------------------------------------------------
    ...
    # save a short result.json (kept simple)
...
def load_model_from_checkpoint(
    ...
    """
    Build model from cfg and load weights from a Lightning checkpoint.

    Mirrors your EMC evaluation script logic:
    - Checkpoint keys look like 'model.backbone.enc_a...' etc.
    - Model expects 'backbone.enc_a...', 'heads.0...', etc.
    ...
    for k, v in state_dict.items():
        ...
        if "backbone." in k:
            ...
            new_key = k[idx:]  # keep from 'backbone.' onwards
        elif "heads." in k:
            idx = k.index("heads.")
            new_key = k[idx:]  # keep from 'heads.' onwards
...
def evaluate_fold_best_epoch(
    ...
    # Parse metrics.csv for fold-level info (best/last auc, epochs)
    if os.path.exists(metrics_path):
        try:
    ...
    # Build validation dataset & dataloader (CPU inference)
    patch_size = cfg["patch_size"]
    angle_range = (-cfg["imaging_aug_deg"], cfg["imaging_aug_deg"])
    mask_pad_threshold = cfg["mask_pad_thresh"]
    dropout_p = cfg["clinical_aug"]
    ...
    # Build model and load best-epoch weights (CPU) using robust key remapping
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            # Make sure we can iterate over centers and sids
    ...
    # Fold-level per-center AUC (based on best-epoch predictions)
...
def main():
    # Global seed for reproducibility of outer process
    GLOBAL_SEED = 17
    ...
    # Build manifest & 3-fold CV splits on dev (AVL + LUMC + RUMC)
    ...
    # For stratification by both outcome and center:
    ...
    # Scenario: 5 days (snellius max) - 5 hours, effectively no trial limit
    walltime_limit_seconds = (5 * 24) * 3600
    scenario = Scenario(
        cs,
        deterministic=True,  # Single-seed evaluation
        n_trials=10 ** 9,  # effectively unbounded, walltime will stop it
        walltime_limit=walltime_limit_seconds,
        seed=GLOBAL_SEED,
        output_directory=SMAC_OUTPUT_DIR,  # <--- fixed place on disk for SMAC state
        name=SMAC_RUN_NAME,  # <--- fixed experiment name to allow resume
    )

    # Initial design with 10 configs
    initial_design = HyperparameterOptimizationFacade.get_initial_design(
        scenario,
        n_configs=10,
    )

    # Intensifier with exactly 1 config call (single seed) per configuration
    intensifier = HyperparameterOptimizationFacade.get_intensifier(
        scenario,
        max_config_calls=1,
    )

    # SMAC objective: for a given config and seed, run 3-fold CV and return cost = 1 - (0.5*AUC_dev + 0.5*center_mean)
    def smac_objective(config: Configuration, seed: int = 0) -> float:
        ...
        for fold_idx, (train_ids, val_ids) in enumerate(cv_splits):
            ...
            print(f"\n[SMAC] Config slug={slug} | seed={seed} | CV fold={fold_idx}")
            report = {k: cfg.get(k) for k in REPORT_KEYS if k in cfg}
            print(f"[SMAC] CONFIG -> {report}")
            ...
            print(f"[SMAC] slug={slug} | seed={seed} | fold={fold_idx} | val_auc={val_auc:.4f}")
            val_aucs.append(val_auc)

            # Evaluate best-epoch checkpoint on validation to get predictions and fold-level summary
        ...
        # Aggregate dev-wide metrics from predictions
        ...
        if len(all_pred_rows) > 0:
            ...
            # No predictions available (something went wrong)
            score = 0.0

        cost = 1.0 - score  # SMAC minimizes cost

        # Save per-config prediction CSV for later analysis
        if len(all_pred_rows) > 0:
            df_pred = pd.DataFrame(all_pred_rows)
        ...
        # Save aggregate results for this (config, seed)
        ...
        print(
            f"[SMAC] slug={slug} | seed={seed} | "
            f"mean_val_auc={mean_val_auc:.4f} | "
            f"AUC_dev={AUC_dev} | center_mean={center_mean} | cost={cost:.4f}"
        )
        return cost

    # Create SMAC object (continuous optimization, overwrite=False)
    smac = HyperparameterOptimizationFacade(
        scenario,
        smac_objective,
        initial_design=initial_design,
        intensifier=intensifier,
        overwrite=False,  # allow continuous runs / resume
    )

    # Optimize (runs until walltime limit or interruption)
    incumbent = smac.optimize()

    # For convenience, save incumbent config as JSON
    incumbent_cfg = cfg_from_config(incumbent, base_cfg)
    incumbent_slug = config_slug(incumbent_cfg)
    with open(os.path.join(runs_root, f"incumbent_{incumbent_slug}.json"), "w") as f:
        json.dump(incumbent_cfg, f, indent=2)

    print("\n[SMAC] Optimization finished.")
    print(f"[SMAC] Incumbent slug: {incumbent_slug}")
    print(f"[SMAC] Incumbent config: {incumbent_cfg}")
